...
const CorporateProfileForm = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            if (data) {
                ...
                setFormData({
                    title: data.title || "Corporate Profile",
                    data: {
    ...
    const handleContentChange = (index, value) => {
        const newContent = [...formData.data.content];
    ...
    const removeParagraph = (index) => {
        const newContent = formData.data.content.filter((_, i) => i !== index);
        setFormData(prev => ({
            ...prev,
            data: { ...prev.data, content: newContent }
        }));
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            await axios.put("/api/content/corporate", formData);
        ...
        <form onSubmit={handleSubmit} className="space-y-8 max-w-4xl">
            ...
            <div>
                ...
                <div className="space-y-4">
                    {formData.data.content.map((para, index) => (
                        <div key={index} className="flex gap-2">
                            ...
                            <button
                                ...
                                disabled={formData.data.content.length === 1}
                            >
                                <X size={20} />
                            </button>
                        </div>
